...
class TestApp:
    ...
    def test_show_articles_route(self):
        ...
        with app.app_context():
            ...
            assert response.status_code != 404  # Ensure response is not "Article not found"
    ...
    def test_increments_session_page_views(self):
        ...
        with app.test_client() as client:
            ...
            assert(session.get('page_views') == 1)

            client.get('/articles/2')
            assert(session.get('page_views') == 2)

            client.get('/articles/3')
            assert(session.get('page_views') == 3)

            client.get('/articles/3')
            assert(session.get('page_views') == 4)
